        ...
        <div className="sticky top-0 bg-white/80 backdrop-blur-lg border-b border-gray-200 p-6 flex justify-between items-center">
          <div>
            ...
            <div className="flex items-center gap-2 mt-2">
              ...
              <span className="text-lg font-semibold text-[#736CED] capitalize">{event.location}</span>
            </div>
            {event.original.createdBy && (
              <div className="flex items-center gap-2 mt-1">
                <Users size={16} className="text-[#736CED] flex-shrink-0" />
                <span className="text-m text-[#736CED] font-semibold">Created By: {event.original.createdBy.firstname} {event.original.createdBy.lastname}</span>
              </div>
            )}
          </div>
          <button
          ...
          {event.type === "booth" && (
            <div className="space-y-4 text-[#312A68]">
              <div className="flex items-start gap-3">
                ...
                <div><span className="font-semibold">Size:</span> {rawEvent.boothSize}</div>
              </div>
              
              {rawEvent.location && (
                <div className="flex items-start gap-3">
                  <MapPin size={16} className="mt-1 text-[#736CED] flex-shrink-0" />
                  <div><span className="font-semibold">Platform Location:</span> {rawEvent.location}</div>
                </div>
              )}
          ...
          {event.type === "workshop" && (
            <div className="space-y-4 text-[#312A68]">
              ...
              <div className="flex items-start gap-3">
                ...
                <div><span className="font-semibold">Register By:</span> {event.registrationDeadline ? localFormatDate(new Date(event.registrationDeadline)) : 'N/A'}</div>
              </div>
              ...
              <div className="flex items-start gap-3">
                <Building2 size={16} className="mt-1 text-[#736CED] flex-shrink-0" />
                <div><span className="font-semibold">Faculty Responsible:</span> {event.faculty}</div>
              </div>
              
              <div className="flex items-start gap-3">
                <Users size={16} className="mt-1 text-[#736CED] flex-shrink-0" />
                <div><span className="font-semibold">Capacity:</span> {event.capacity} attendees</div>
              </div>
              ...
              {event.extrarequiredfunding > 0 && (
                <div className="flex items-start gap-3">
                  ...
                  <div><span className="font-semibold">Extra Required Funding:</span> {event.extrarequiredfunding.toLocaleString()} EGP</div>
                </div>
              )}

              {event.professors && event.professors.length > 0 && (
                <div className="flex items-start gap-3">
                  <UserCheck size={16} className="mt-1 text-[#736CED] flex-shrink-0" />
                  <div>
                    <span className="font-semibold">Participating Professors:</span>
                    <ul className="mt-2 space-y-1">
                      {event.professors.map((prof, index) => (
                        <li key={index} className="text-sm bg-gray-50 p-2 rounded">
                          <strong>• {prof.firstname} {prof.lastname}</strong> {prof.email && `(${prof.email})`}
                        </li>
                      ))}
                    </ul>
                  </div>

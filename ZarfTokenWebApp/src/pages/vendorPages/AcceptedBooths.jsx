            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-60 backdrop-blur-sm animate-fade-in">
                <div className="bg-white rounded-2xl w-full max-w-2xl shadow-2xl m-4 max-h-[90vh] overflow-y-auto transform animate-slide-up">
                    <div className="sticky top-0 bg-white/80 backdrop-blur-lg border-b border-gray-200 p-6 flex justify-between items-center">
                        <div>
                            <h2 className="text-2xl font-bold text-[#4C3BCF]">{booth.bazarId?.bazaarname}</h2>
                            <p className="text-sm text-[#312A68] flex items-center gap-2 mt-1"><MapPin size={14} /> {booth.bazarId?.location}</p>
                        </div>
                    ...
                    <div className="p-6 space-y-6">
                        <div className="space-y-4">
                            <div className="flex items-start gap-3">
                                ...
                                <div><span className="font-semibold">Booth Name:</span> {booth.boothname}</div>
                            </div>
                            <div className="flex items-start gap-3">
                                <Info size={16} className="mt-1 text-[#736CED] flex-shrink-0" />
                                <div><span className="font-semibold">Size:</span> {booth.boothSize}</div>
                            </div>
                            {booth.location && (
                                <div className="flex items-start gap-3">
                                    <MapPin size={16} className="mt-1 text-[#736CED] flex-shrink-0" />
                                    <div><span className="font-semibold">Location:</span> {booth.location}</div>
                                </div>
                            )}
                            {booth.bazarId?.startdate && (
                                <div className="flex items-start gap-3">
                                    <Calendar size={16} className="mt-1 text-[#736CED] flex-shrink-0" />
                                    <div><span className="font-semibold">Starts:</span> {formatDateTime(booth.bazarId.startdate, booth.bazarId.starttime)}</div>
                                </div>
                            )}
                            {booth.bazarId?.enddate && (
                                <div className="flex items-start gap-3">
                                    <Clock size={16} className="mt-1 text-[#736CED] flex-shrink-0" />
                                    <div><span className="font-semibold">Ends:</span> {formatDateTime(booth.bazarId.enddate, booth.bazarId.endtime)}</div>
                                </div>
                            )}
            ...
            <div className="bg-white rounded-2xl w-full max-w-2xl shadow-2xl m-4 max-h-[90vh] overflow-y-auto transform animate-slide-up">
                <div className="sticky top-0 bg-white/80 backdrop-blur-lg border-b border-gray-200 p-6 flex justify-between items-center">
                    <div>
                        ...
                        <p className="text-sm text-[#312A68] flex items-center gap-2 mt-1"><Globe size={14} /> Platform Storefront</p>
                    </div>
                    <button onClick={onClose} className="p-2 rounded-full text-gray-500 hover:bg-gray-100 hover:text-gray-800 transition-colors"><X size={24} /></button>
                </div>
                <div className="p-6 space-y-6">
                    <div className="space-y-4">
                        ...
                        <div className="flex items-start gap-3">
                            <Clock size={16} className="mt-1 text-[#736CED] flex-shrink-0" />
                            <div><span className="font-semibold">Duration:</span> {booth.duration} weeks</div>
                        </div>
                        {booth.location && (
                            <div className="flex items-start gap-3">
                                <MapPin size={16} className="mt-1 text-[#736CED] flex-shrink-0" />
                                <div><span className="font-semibold">Platform Location:</span> {booth.location}</div>
                            </div>
                        )}
                        {booth.people && booth.people.length > 0 && (
                            <div className="flex items-start gap-3">
  ...
  useEffect(() => {
    const fetchBooths = async () => {
      try {
        ...
        const vendorBooths = response.data.filter(booth => 
          (booth.vendorId === user._id || booth.vendorId?._id === user._id) &&
          ...
          <main className="flex w-full flex-1 flex-col items-center px-4 sm:px-6 py-8">
            <div className="w-full max-w-6xl">
              <div className="mb-8 text-center">
                ...
                <p className="text-md text-[#312A68] mt-2">Manage your accepted booths and platform storefronts.</p>
              </div>
              ...
              {loading ? (
                <div className="text-center py-16"><p>Loading your booths...</p></div>
              ) : error ? (
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  {filteredItems.map((booth) => (
                    booth.isBazarBooth ? (
                      <div key={booth._id} className="bg-white rounded-2xl p-6 shadow-lg flex flex-col">
                        {/* Bazaar Card content */}
                        <div>
                          <h3 className="text-xl font-bold text-[#4C3BCF]">{booth.bazarId?.bazaarname || 'Bazaar Booth'}</h3>
                        ...
                        <div className="mt-auto pt-4 border-t">
                            <button onClick={() => setSelectedBooth(booth)} className="text-sm font-semibold text-[#736CED] hover:text-[#4C3BCF] transition-colors">View Details</button>
                        </div>
                      </div>
                    ) : (
                      <div key={booth._id} className="bg-white rounded-2xl p-6 shadow-lg flex flex-col">
                        {/* Platform Card content */}
                         <div>
                          <h3 className="text-xl font-bold text-[#4C3BCF]">{booth.boothname}</h3>
                           <p className="text-sm text-[#312A68] mt-1">Your long-term virtual storefront</p>
                           <div className="flex items-center gap-4 mt-3 text-sm text-[#736CED]">
                             <span className="flex items-center gap-1">
                               <Users size={14} />
                               {booth.boothSize}
